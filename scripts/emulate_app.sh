...
if [[ $create_snapshot == true ]]; then
    ...
    emulator -avd "${avd_name}" -verbose -netdelay none -netspeed full -no-window -no-audio -no-boot-anim -no-snapshot-load -port 5554  | tee "${snapshot_report_dir}/emulator.txt"
    ...
    echo -e "\n\nWaiting for emulator to launch..."
    emulator -avd "${avd_name}" -netdelay none -netspeed full -no-boot-anim -no-snapshot-save -read-only -port 5554 | tee "${test_report_dir}/emulator.txt" &
    wait_for $timeout_seconds check_launch
    echo "$(adb devices | grep "emulator-5554" | cut -f1) launch successful"
    ...
    yarn react-native start &> "${test_report_dir}/react_native.out" &
